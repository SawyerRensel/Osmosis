...
**Description**: Osmosis provides a 3-layer styling and view state architecture:
...
Individual notes override theme defaults via frontmatter under the `osmosis:` key:
...
Overrides travel with the note (portable, git-friendly). Node targeting uses a **hybrid B+C approach**:
...
#### Transclusion Style Cascade

When note A transcludes note B, styles resolve via CSS-like cascading:

```
Theme defaults → Note B's frontmatter styles → Note A's frontmatter overrides
```

1. Transcluded content inherits the **host map's theme** by default
2. Note B's per-node overrides are **preserved** (B's diamond stays a diamond)
3. Note A can **override** transcluded styles by targeting `"![[Note B]]/## Intro"`
4. View state for transcluded content **belongs to the host** (A's view file, not B's)
...
A sidebar panel for interactive styling: select a node → see shape, fill, border, text, branch options → changes preview in real-time → stored as frontmatter overrides with stable IDs.
...
- [ ] Format panel allows interactive per-node styling with live preview
- [ ] Colored branches toggle auto-assigns distinct branch colors with child inheritance
- [ ] Transclusion cascade resolves correctly (host theme → embedded overrides → host overrides)
...
**Constraints**: Styling metadata in frontmatter must not interfere with other Obsidian plugins or standard frontmatter. View state files must be lightweight (< 50 KB for a 2,000-node map).

**Priority**: Must-Have (theme system + basic per-node styling for v1.0; format panel, named view states, and CSS snippets for v1.1)
**Estimated Complexity**: Medium
...
| Mind map theme system + basic styling | Yes | Medium | v1.0 | 3-layer architecture, preset themes, per-node overrides |
| Mind map view state persistence | Yes | Low | v1.0 | Explicit save of fold/pan/zoom to sidecar files |
| Format panel + named view states | | Medium | v1.1 | Interactive styling GUI, multiple saved views |
...
- Theme system with preset themes + per-note style overrides (frontmatter)
- View state persistence (fold/pan/zoom saved to sidecar files)

**Phase 2 — v1.1:**
- Timeline view layout
- Format panel (interactive per-node styling GUI with live preview)
